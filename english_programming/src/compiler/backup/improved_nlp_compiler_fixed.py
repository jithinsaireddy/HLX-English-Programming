...
class ImprovedNLPCompiler(EnhancedNLCompiler):
    ...
    def normalize_with_nlp(self, line: str) -> str:
        ...
        create_var_match = re.match(r"create a variable called (\w+) and set it to (.+)", line.lower())
        if create_var_match:
        ...
        concat_match = re.match(concat_pattern, line.lower())
        if concat_match:
        ...
        write_match = re.match(r"write \"(.+?)\" to file (.+)", line.lower())
        if write_match:
            content = write_match.group(1)
            filename = write_match.group(2)
            return f"WRITEFILE \"{content}\" file {filename}"
        
        read_match = re.match(r"read file (.+) and store (?:lines|contents) in (\w+)", line.lower())
        if read_match:
